...
namespace vericard
{
   ...
   bool animation_disco::initialize()
   {
      ...
      ::file::path path = ::dir::system() / "config/resident/person.gif";

      if(!m_dibPerson.load_from_file(path, false))
      {
   ...
   bool animation_disco::reset()
   {
      ...
      for(int i = 0; i < Application.veripack().m_commptra.get_count(); i++)
      {

         auto comm = Application.veripack().m_commptra[i];
         ...
         for(int j = 0 ; j < straUsers.get_count(); j++)
         {
            ...
            sp(::vericard::user) puser = comm->get_user(straUsers[j], false);
   ...
   void animation_disco::defer_add_llama()
   {
      ...
      l.m_iOffset = -m_dibPerson->m_size.cx * 2.0;
      l.m_dColor = System.math().frand_range(0.0, 2 * 3.1415);
      l.m_dColorSpeed = System.math().frand_range(0.013, 0.04);
      l.m_rect = ::rect(null_point(), m_dibPerson->m_size * l.m_dSize);
      l.m_dib.alloc(allocer());
      l.m_dib->create(l.m_rect.get_size());
      l.m_dibHead.alloc(allocer());
      l.m_dibHead->create(l.m_rect.get_size());
      m_llama.add(l);
   ...
   void animation_disco::_001OnDrawBefore(::draw2d::graphics * pgraphics)
   {
...

      {
         ...
         for (index i = 0; i < m_llama.get_count(); i++)
         {
            ...
            synch_lock sl(puser != NULL ? puser->m_pmutex : NULL);
            ...
            if(pdib != NULL)
            {
               ...
               rHead.left = 0;
               rHead.top = 0;
               rHead.right = pdib->m_size.cx * l.m_dSize * 1.2;
               rHead.bottom = pdib->m_size.cy * l.m_dSize * 1.2;

               rHead.Align(align_center, rAlign);

               //pdib->m_dSpeed = l.m_dSpeed;

               //pdib->defer_update();

            }

            m_dibPerson.m_dSpeed = l.m_dSpeed;

            m_dibPerson.defer_update();
            ...
            l.m_dib->get_graphics()->StretchBlt(0, 0,
                                                ...
                                                m_dibPerson->get_graphics(), 0, 0, m_dibPerson->m_size.cx, m_dibPerson->m_size.cy);
            ...
            if (pdib != NULL)
            {
               ...
               for (index i = 0; i < m_dibPerson->m_size.cy; i++)
               {

                  if (argb_get_a_value(m_dibPerson->pixel(m_dibPerson->m_size.cx / 2, i)) > 0x33)
                  {
               ...
               l.m_dib->get_graphics()->set_alpha_mode(::draw2d::alpha_mode_blend);

               l.m_dib->get_graphics()->StretchBlt(rHead.left -rDisco.left, MAX(0, iFirst * l.m_dSize - 5),
                                                   rHead.width(), rHead.height(),
                                                   pdib->get_graphics(), 0, 0, pdib->m_size.cx, pdib->m_size.cy);
            ...
            l.m_iOffset += m_dibPerson->m_size.cx * l.m_dSize*l.m_dSpeed / 30.0;
